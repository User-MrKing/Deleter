...
//  线程函数
DWORD WINAPI GetHistoryFile(LPVOID lpParameter);
DWORD WINAPI GetHistoryProjectAndSolution(LPVOID lpParameter);
...
BOOL CDeleterDlg::OnInitDialog()
{
	...
	v_lstView.InsertColumn(1, TEXT("路径"), 0, 490);

	HANDLE hThreadF = CreateThread(NULL, 0, GetHistoryProjectAndSolution, this, 0, NULL);
	...
	if (lRet == ERROR_SUCCESS) {
		...
		if (lRet == ERROR_SUCCESS) {
			...
			for (DWORD dwIndex = 0; dwIndex < dwValueCount; ++dwIndex) {
				...
				dlg->v_lstView.InsertItem(dwIndex, strValueName);
				dlg->v_lstView.SetItemText(dwIndex, 1, strValueData);
			}
			delete[] pszName;
			delete[] lpValueData;
			RegCloseKey(hKey);
